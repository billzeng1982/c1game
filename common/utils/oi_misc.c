...
char *DateTimeStr(time_t *mytime)
{
	...
	{
		sprintf(s, "%04d-%02d-%02d %02d:%02d:%02d", 
            CurrTm.tm_year+2000, CurrTm.tm_mon+1, CurrTm.tm_mday,
            CurrTm.tm_hour, CurrTm.tm_min, CurrTm.tm_sec);
	}
...
char *DayStr(time_t mytime)
{
	...
	sprintf(s, "%02d%02d", CurrTm.tm_mon+1, CurrTm.tm_mday);
	return s;
}
...
long sec, usec;
	...
	if (iFlag) {
		...
		va_end(ap);
		sprintf(sStr, "%s: %ldsec %ldms %ldus", sTmp, sec, usec / 1000, usec % 1000);
...
prpsinfo_t prpsinfo;
	...
	for (i = 0; i < 20; i++) {
		// May PID reuse immediately?
		// if (kill(iPid, SIGTERM) < 0) {
		if (kill(iPid, SIGUSR1) < 0) {
			switch (errno) {
				case ESRCH:
				printf("killed.\n");
					return 0;
...
int iBytesRead;
	...
	if ( NULL != (pf1=fopen(sFile1, "r")) ) {
		if ( NULL != (pf2=fopen(sFile2, "w")) ) {
...
int CheckCharacter ( char *pszStr )
{
    ...
    for ( p = (unsigned char*)pszStr ; *p ; p ++ )
    {
...
#define SPECIFIC_SYMBOL_NUM 10
char szMergeWordAddSymbol[SPECIFIC_SYMBOL_NUM] = {'@', '#', '$', '%', '&', '!', '+', '^', '*', '~'};

int CheckSpecificSymbol(char cSymbol)
{
    int i = 0;
    for (i =0; i < SPECIFIC_SYMBOL_NUM; i++)
    {
        if (cSymbol == szMergeWordAddSymbol[i])
        {
            return 1;
        }
    }
...
/*
	*piEqual==0，表示没有找到，返回值表示应该在数组中插入的位置
	*piEnqual==1,表示找到，返回值为数组下标
*/
