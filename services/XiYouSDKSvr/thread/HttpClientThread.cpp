...
const char* SDK_HTTP_HEADER = "Content-Type: application/x-www-form-urlencoded";
...
bool HttpClientThread::_ThreadAppInit(void* pvAppData)
{
    ...
    m_oHttpClient.HeaderAppend(SDK_HTTP_HEADER);
    m_oMsgLayer.Init();    
...
int HttpClientThread::_ThreadAppProc()
{
    ...
    //IMsgBase* poMsgHandler = XiYouSDKSvrMsgLayer::Instance().GetMsgHandler(m_stSsRecvPkg.m_stHead.m_wMsgId);
