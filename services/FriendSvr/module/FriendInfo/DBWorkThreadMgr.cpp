...
bool DBWorkThreadMgr::Init(FRIENDSVRCFG*	m_pstConfig)
{
	//初始化工作线程
...
//发送请求
void DBWorkThreadMgr::SendReq(DT_FRIEND_DB_REQ& rstDBReq)
{
	int iIndex = 0;
	if (0 == rstDBReq.m_stWholeData.m_stBaseInfo.m_ullUin)
	{
		iIndex = THREAD_INDEX(rstDBReq.m_stWholeData.m_stBaseInfo.m_szName[0]);

	}
	else
	{
		iIndex = THREAD_INDEX(rstDBReq.m_stWholeData.m_stBaseInfo.m_ullUin);
	}
	m_astWorkThreads[iIndex].SendReq(rstDBReq);

}		

void DBWorkThreadMgr::HandleRspMsg()
{
	for (int i=0; i< m_iWorkThreadNum; i++)
	{
		...
		FriendMgr::Instance().HandleDBThreadRsp(m_stDBRsp);
	}
}
