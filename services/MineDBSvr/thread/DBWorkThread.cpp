...
void CDBWorkThread::SendToMineSvr(PKGMETA::SSPKG& rstSsPkg)
{
    rstSsPkg.m_stHead.m_iDstProcId = m_pstConfig->m_iMineSvrID;
...
{        
    m_pstConfig = (MINEDBSVRCFG*)pvAppData;
    m_szBinBuf = new char[CONST_MINE_ORE_INFO_BINBUF_SIZE];
    if ( m_szBinBuf == NULL)
    {
        LOGERR_r("new char[CONST_MINE_ORE_INFO_BINBUF_SIZE] failed!");
        return false;
    }
    // init mysql hanlder
    ...
    m_oMsgLayer.Init();
    m_oMineOreTable.Init(&m_oMysqlHandler, m_pstConfig, m_szBinBuf);
    m_oMinePlayerTable.Init(&m_oMysqlHandler, m_pstConfig, m_szBinBuf);
    return true;
}
