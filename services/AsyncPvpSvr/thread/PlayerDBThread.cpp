...
PlayerDBThread::PlayerDBThread() : m_stSendBuf(sizeof(DT_ASYNC_PVP_PLAYER_DB_REQ)*2+1)
{

}

void PlayerDBThread::SendReqPkg(DT_ASYNC_PVP_PLAYER_DB_REQ& rstDBReq)
{
    TdrError::ErrorType iRet = rstDBReq.pack(m_stSendBuf.m_szTdrBuf, m_stSendBuf.m_uSize, &m_stSendBuf.m_uPackLen );
    if( iRet != TdrError::TDR_NO_ERROR)
    {
        LOGERR_r("pack DT_ASYNC_PVP_PLAYER_DB_REQ pkg error! operate type(%d)", rstDBReq.m_bOpType);
        return;
    }
...
void PlayerDBThread::SendRspPkg(DT_ASYNC_PVP_PLAYER_DB_RSP& rstDBRsp)
{
    TdrError::ErrorType iRet = rstDBRsp.pack(m_stSendBuf.m_szTdrBuf, m_stSendBuf.m_uSize, &m_stSendBuf.m_uPackLen );
    if( iRet != TdrError::TDR_NO_ERROR)
    {
        LOGERR_r("pack DT_ASYNC_PVP_PLAYER_DB_RSP pkg error! operate type(%d)", rstDBRsp.m_bOpType);
        return;
    }
...
bool PlayerDBThread::_ThreadAppInit(void* pvAppData)
{
    ...
    if (!m_oPlayerTable.CInit(ASYNCPVP_PLAYER_TABLE_NAME, &m_oMysqlHandler, pstConfig))
    {
...
int PlayerDBThread::_HandleMsg()
{
    ...
    switch(m_stReqPkg.m_bOpType)
    {
        //Update和Delete不用回复
        case DB_UPDATE:
            iOpRet = m_oPlayerTable.UpdatePlayer(m_stReqPkg.m_stData);
            return 1;
        case DB_DEL:
            iOpRet = m_oPlayerTable.DeletePlayer(m_stReqPkg.m_stData.m_stShowdata.m_stBaseInfo.m_ullUin);
            return 1;
        case DB_UPDATE_PART:
            iOpRet = m_oPlayerTable.GetPlayer(m_stReqPkg.m_stData.m_stShowdata.m_stBaseInfo.m_ullUin, m_stRspPkg.m_stData);
            if (iOpRet==ERR_NONE)
            {
                m_stRspPkg.m_stData.m_stShowdata = m_stReqPkg.m_stData.m_stShowdata;
                m_oPlayerTable.UpdatePlayer(m_stRspPkg.m_stData);
            }
            return 1;
        case DB_CREATE:
            ...
            break;
        case DB_GET:
            iOpRet = m_oPlayerTable.GetPlayer(m_stReqPkg.m_stData.m_stShowdata.m_stBaseInfo.m_ullUin, m_stReqPkg.m_stData);
            break;
        default:
            LOGERR_r("PlayerDBThread handle msg error, msg type(%d) is error", m_stReqPkg.m_bOpType);
            return -3;
    }
