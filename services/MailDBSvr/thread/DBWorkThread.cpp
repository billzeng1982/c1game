...
{        
    m_pstConfig = (MAILDBSVRCFG*)pvAppData;
    ...
    m_oMailPriTable.CInit( MAIL_PRI_TABLE_NAME, &m_oMysqlHandler, m_pstConfig);
    m_oMailPubTable.CInit( MAIL_PUB_TABLE_NAME, &m_oMysqlHandler, m_pstConfig);
...
int CDBWorkThread::_HandleMailSvrMsg()
{
    ...
    //IMsgBase* poMsgHandler = MailDBSvrMsgLayer::Instance().GetMsgHandler(m_stSsRecvPkg.m_stHead.m_wMsgId);
