...
class GuildExpeditionGuildTable
{

public:
	GuildExpeditionGuildTable();
	virtual ~GuildExpeditionGuildTable();

	virtual bool Init(CMysqlHandler* poMysqlHandler, const char* pstTableName, CLUSTERDBSVRCFG* m_pstConfig, char* szBinBuff);
	int UptData(PKGMETA::DT_GUILD_EXPEDITION_GUILD_DATA* pstData, int8_t bNum);
	int GetData(uint64_t ullKeyList[], int8_t bNum, OUT PKGMETA::DT_GUILD_EXPEDITION_GUILD_DATA* pstData, OUT int8_t* pOutNum);
