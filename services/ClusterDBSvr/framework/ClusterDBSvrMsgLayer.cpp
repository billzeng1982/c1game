...
#include "ClusterDBSvrMsgLayer.h"



using namespace PKGMETA;

ClusterDBSvrMsgLayer::ClusterDBSvrMsgLayer() : CMsgLayerBase(sizeof(PKGMETA::SCPKG) * 2, sizeof(PKGMETA::SSPKG) * 2)
{
...
bool ClusterDBSvrMsgLayer::Init()
{
	m_pstConfig = &(ClusterDBSvr::Instance().GetConfig());
...
int ClusterDBSvrMsgLayer::DealPkg()
{
    int iRecvBytes = 0;
    int iDealPkgNum = 0;
...
bool ClusterDBSvrMsgLayer::Fini()
{
    for (int i = 0; i < m_pstConfig->m_iWorkThreadNum; i++)
    {
...
void ClusterDBSvrMsgLayer::_ForwardToWorkThread(MyTdrBuf* pstTdrBuf)
{
    assert(pstTdrBuf);
